...
const stateParam = computed(() => {
  return segments.value.find(seg =>
     loadstates.some(
      s => normalize(s.name) === normalize(seg)
    )
  ) || ''
})
...
//     .trim()
//     .toLowerCase()
//     .replace(/[-\s]+/g, '_')   // handle space + dash
//     .replace(/[^a-z0-9_]/g, '')
// }
...
console.log(stateParam,'vfffdfgddfgdfgdfgdfg');
...
const cityParam = computed(() => {
  ...
  return segments.value.find(seg =>
    cityOptions.value.some(
      city => normalize(city) === normalize(seg)
    )
  ) || ''
})

const typeParam = computed(() =>
  segments.value.find(s => ['land','house'].includes(s)) || ''
)
...
// const cityParam = computed(() =>
//   segments.value.find(s =>
//     s !== stateParam.value &&
//     s !== typeParam.value &&
//     s !== categoryParam.value
//   ) || ''
// )
...
/* =================================
   🔥 SYNC ROUTE → FILTER STATE (ONE WATCH ONLY ✅)
================================= */
watch(
  () => route.fullPath,
  () => {
...
watchEffect(() => {
  ...
  console.log('CATEGORY PARAM:', categoryParam.value)
...
const isActive = (c) => {
  ...
  if (c === 'Land') return typeParam.value === 'land'
  if (c === 'House') return typeParam.value === 'house' && !categoryParam.value
  if (c === 'Flat / Apartment') {
    return typeParam.value === 'house' && categoryParam.value === 'flat'
  }
  return categoryParam.value === c.toLowerCase()
}
...
const getPriceLabel = (item) => {
  ...
  if (item?.type === 'house') {
    if (item?.purpose === 'sale') {
      if (pricing.paymentType === 'installment') return '/month'
      return '/outright'
    }
